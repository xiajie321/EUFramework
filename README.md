...
- 目前框架的Readme文档与API文档为AI书写(因为目前需要专注于功能模块的开发,文档临时使用的AI去填充主要作用为开发者备忘)。

# 框架简介(临时)

本框架是专门服务于2D与2.5D开发者的高性能高效的OOP开发框架，分层来自于qf框架并对其核心逻辑进行重构，尽量去避免框架本身带来的GC开销，使用更加高效的分层通信方式，大部分扩展工具可以使用类似Unity的包管理器去管理卸载与装载，大部分EU框架内置的工具都会进行多线程优化，框架的大部分工具都可以单独拿出来使用，方便去更换与集成。
