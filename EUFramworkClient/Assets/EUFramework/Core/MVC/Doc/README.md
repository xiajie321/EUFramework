...
- **极简主义设计**：基于 IOC + MVC 的分层架构，结构清晰，上手简单。
- **高性能事件系统**：内置 `TypeEventSystem`，采用静态泛型缓存 + 数组存储，实现**零 GC (Zero-GC)** 的事件发送与监听。
- **生命周期管理**：严格的模块初始化 (Init) 与销毁 (Dispose) 顺序管理，确保依赖关系正确。
  - 初始化：Utility -> Model -> System (基础设施 -> 数据 -> 逻辑)
  - 销毁：System -> Model -> Utility (逻辑 -> 数据 -> 基础设施)
- **泛型与扩展**：通过 `CoreExtension` 提供丰富的 `this.GetModel<T>()` 等扩展方法，编码流畅且类型安全。
